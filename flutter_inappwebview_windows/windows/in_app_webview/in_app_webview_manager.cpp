...
namespace flutter_inappwebview_plugin
{
  ...
  void InAppWebViewManager::createInAppWebView(const flutter::EncodableMap *arguments, std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
  {
    ...
    auto hwnd = CreateWindowEx(0, windowClass_.lpszClassName, L"", dwStyle, 0,
    ...
    auto webViewEnvironment = webViewEnvironmentId.has_value() && map_contains(plugin->webViewEnvironmentManager->webViewEnvironments, webViewEnvironmentId.value())
                                  ? plugin->webViewEnvironmentManager->webViewEnvironments.at(webViewEnvironmentId.value()).get()
                                  : nullptr;
    ...
    InAppWebView::createInAppWebViewEnv(hwnd, true, webViewEnvironment, initialSettings,
                                            ...
                                            wil::com_ptr<ICoreWebView2CompositionController> webViewCompositionController)
                                        {
                                          ...
                                          {
                                            std::optional<std::vector<std::shared_ptr<UserScript>>> initialUserScripts = initialUserScriptList.has_value() ? functional_map(initialUserScriptList.value(), [](const flutter::EncodableValue &map)
                                                                                                                                                                            { return std::make_shared<UserScript>(std::get<flutter::EncodableMap>(map)); })
                                            ...
                                            HRESULT hr = inAppWebView->webViewController->get_CoreWebView2(&coreWebView2);
                                            if (SUCCEEDED(hr))
                                            {
                                              // UI Automation을 제어하기 위한 인터페이스 접근
                                              Microsoft::WRL::ComPtr<IUIAutomation> pAutomation;
                                              hr = CoCreateInstance(CLSID_CUIAutomation, nullptr, CLSCTX_INPROC_SERVER, IID_IUIAutomation, (void **)&pAutomation);
                                              if (SUCCEEDED(hr))
                                              {
                                                // 컨트롤러에서 IUIAutomationElement 얻기
                                                Microsoft::WRL::ComPtr<IUIAutomationElement> pElement;
                                                hr = pAutomation->ElementFromHandle(hwnd, &pElement);
                                                if (SUCCEEDED(hr))
                                                {
                                                  // UIA_IsProgrammaticallyHiddenPropertyId를 false로 설정 (이 부분은 실제로는 동작하지 않습니다. UI Automation은 내부적으로 관리됩니다.)
                                                  VARIANT var;
                                                  var.vt = VT_BOOL;
